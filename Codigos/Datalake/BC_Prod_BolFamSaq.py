...
# Autor: Bruno Jardim                                                       #
# Data: 16/03/2019                                                          #
# Assunto: Beneficios aos Cidadaos - Bolsa Familia - Saques                 #
# Defasagem: M - 4                                                          #  
#############################################################################
#############################################################################
...
print('------- Bolsa Familia - Saques -----------')
print('##########################################')
#############################################################################
# ------------------------ INICIO FUNCOES -----------------------------------

def CetusLakeBeneficiosCidadaoBolsaFamiliaSaques(dataproc,ANOMES_INFO,exec_lake = 'prod'):
    ...
    var_url = 'http://www.portaltransparencia.gov.br/download-de-dados/bolsa-familia-saques/'+str(ANOMES_INFO)
    ...
    nm_file1 = str(ANOMES_INFO)+'_BolsaFamilia_Saques.csv'
...
def CetusSalvaCSVS3(dataframe,nm_s3_file,s3_path,encode_df):
    ...
    try:
        bytes_to_write = dataframe.to_csv(sep = ',',header=True, index=False, index_label=None,encoding='utf-8').encode()
    except:
...
print('Safra inicial: ',refstart)
print('Safra inicial: ',refend)

vetor_safras_aux = CetusRefVecDays(refstart,refend) 
#s3_path = 'projeto-bigdata-cetus/datalake/xxxxxxxxxxxx/'

anomes_ = []
for data_ref in vetor_safras_aux:    
    anomes_.append(data_ref.strftime("%Y%m"))
    
vetor_safras = list(dict.fromkeys(anomes_))


for anomes in vetor_safras:
    start_time = time.clock()
    print('Cetus - Carga Historica Ref ---->', anomes)
    dados,encode_df,datinfo = CetusLakeBeneficiosCidadaoBolsaFamiliaSaques(diamesano,anomes,exec_lake = 'Hist')
    nm_s3_file = 'BC_Lake_BolFamPag_'+str(datinfo)+'.csv'
    s3_path = 'projeto-bigdata-cetus/datalake/BeneficiosCidadaos/'
    print('Salvando tabela no lake ---> ',nm_s3_file)
    CetusSalvaCSVS3(dados,nm_s3_file,s3_path,encode_df)
